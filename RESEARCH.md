...
**Core or periphery?** The two perp constructions sit on opposite sides of the boundary (DESIGN.md §3.1):

- **Path 1 — leveraged spot (the "looped" perp above). Implemented:** [`src/periphery/LeverageRouter.sol`](src/periphery/LeverageRouter.sol), together with the `onBehalfOf` owner parameter on `open` (the one core accommodation). Rather than literal iterative looping, the router uses v4 flash accounting — take the debt token, swap it into collateral through the same pool, open one position owned by the trader, settle the flash with the disbursed loan — reaching $\lambda = 1/(1-\mathrm{LTV})$ in a single transaction with a single liquidation range. Every position it creates is an ordinary TrueLend loan, liquidated by the existing chunk engine with no new liquidation code; the truncated oracle's worse-of pricing values the collateral at the pre-swap price, so the construction cannot lever against its own flash-swap impact. No funding rate exists; leverage is expressed through real spot exposure.
- **Path 2 — a true funding-rate perp. Implemented as the sibling repo `TruePerp` (v0.1):** a `TruePerpHook` with hook-custodied margin, worse-of pricing at entry *and* exit, skew-priced funding (crowded side pays; the imbalance residual compensates the LP vault), and cash-settled chunked auto-deleveraging over a margin-derived range — pacing, ranges, filter, and trigger bitmap all reused from this repo's libraries via a submodule. The perp-shaped termination rule falls out naturally: where a loan's decay ends at debt = 0, ADL ends when equity again covers maintenance on the reduced notional — the position survives smaller and healthy. TrueLendHook itself does not change: it has neither the byte budget nor any reason to host perp state.

The asset that transfers in both cases is the liquidation kernel; the perp-specific work is margin bookkeeping and product framing, not a new liquidation design.
