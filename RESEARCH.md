...
- **Path 1 — leveraged spot (the "looped" perp above).** Pure periphery plus one core accommodation. The LeverageRouter loops open → swap → redeposit within a single transaction to reach $\lambda = 1/(1-\mathrm{LTV})$; every position it creates is an ordinary TrueLend loan, liquidated by the existing chunk engine with no new liquidation code. The only core change it needs is an `onBehalfOf` owner parameter on `open` (so looped positions belong to the trader, not the router) — the same one-argument change a convenience router needs anyway. No funding rate exists; leverage is expressed through real spot exposure.
