...
- **Path 2 — a true funding-rate perp.** New **core**, but a *sibling*, not an extension: a PerpHook with margin accounts, funding accrual against the truncated-oracle price, and cash settlement is its own hook contract on its own pools. It reuses the deployed library singletons as-is — `ChunkMath` paces margin liquidations, `LiqRangeMath` places margin-call ranges, `TruncatedOracle` supplies the mark/index spread, `TriggerIndex` finds crossings — because they are linked external libraries with on-chain addresses, not compiled-in code. TrueLendHook itself does not change: it has neither the byte budget (132 bytes of headroom) nor any reason to host perp state.
