...
contract TrueLendHook is BaseHook {
        ...
        for (uint i = 0; i < ticks.length; i++) {
            ...
            if (crossedIntoRange) {
                ...
                for (uint j = 0; j < posIds.length; j++) {
                    ...
                    if (pos.isActive && !pos.needsLiquidation && newTick >= pos.tickLower) {
    ...
    function _calculateChunkSize(
        ...
        uint256 timeMultiplier = (timeSinceLastChunk * 10000) / CHUNK_TIME_INTERVAL;
        ...
        if (currentTick >= pos.tickLower) {
            ...
            depthIntoRange = (uint256(uint24(depthTicks)) * 10000) / uint256(uint24(rangeWidth));
        }
        ...
        if (chunkSize > pos.collateralRemaining) chunkSize = pos.collateralRemaining;
    ...
    function _executeSingleChunk(
        ...
        poolManager.unlock(
            abi.encode(LiquidationSwapData({
                poolKey: key,
    ...
    function unlockCallback(bytes calldata data) external onlyPoolManager returns (bytes memory) {
        LiquidationSwapData memory swapData = abi.decode(data, (LiquidationSwapData));
        ...
        (, int24 currentTick,,) = poolManager.getSlot0(swapData.poolKey.toId());
    ...
    function _calculatePenalty(
        ...
        uint256 penalty = ethReceived
            ...
            .mulDivDown(ltFactor, 10000)              // Multiply by LT factor
            .mulDivDown(timeFactor, 10000);           // Multiply by time factor
        
        return penalty;
    }

    function _checkPositionStatus(PoolKey calldata key, bytes32 positionId) internal {
        ...
        if (pos.debtRepaid >= totalDebt) {
            ...
            emit PositionFullyLiquidated(
                ...
                pos.debtRepaid,
                pos.collateralRemaining  // Return remaining collateral to borrower
            );
        }
    }
    ...
    function createPosition(
        ...
        if (collateralAmount == 0 || debtAmount == 0) revert InsufficientCollateral();
        if (liquidationThreshold > 99 || liquidationThreshold < 50) revert InvalidLiquidationThreshold();
        
        // Get current price and tick
        (uint160 sqrtPriceX96Current, int24 currentTick,,) = poolManager.getSlot0(key.toId());
    ...
    function _calculateLiquidationTicks(
        ...
        uint160 sqrtPriceUpper = uint160((uint256(sqrtPriceLiquidation) * 14142) / 10000);
    ...
    function _estimateNewTick(
        ...
        // Very rough estimate
        int24 estimatedMove = int24(params.amountSpecified / int256(uint256(liquidity)) / 1000);
        return params.zeroForOne ? currentTick - estimatedMove : currentTick + estimatedMove;
    }

    function _calculateTotalDebt(BorrowPosition storage pos) internal view returns (uint256) {
        // Calculate debt with simple accrued interest
        uint256 timeElapsed = block.timestamp - pos.creationTime;
        uint256 interest = pos.debtAmount.mulDivDown(pos.interestRate * timeElapsed, 10000 * 365 days);
        return pos.debtAmount + interest;
    }

    // ============ View Functions ============

    function getPosition(bytes32 positionId) external view returns (BorrowPosition memory) {
        return positions[positionId];
    }

    function getActivePositions(PoolKey calldata key) external view returns (bytes32[] memory) {
