...
2. **[`LeverageRouter`](../src/periphery/LeverageRouter.sol)** — the leveraged-spot construction (RESEARCH.md appendix, path 1): inside one PoolManager unlock it flash-takes the debt token, swaps it into collateral through the same pool, opens a single ordinary position owned by the trader, and settles the flash with the loan the hook disburses — exposure λ = 1/(1−LTV) with no new liquidation code and no funding rate. Closing reverses it: flash **one wei more** than the live debt (the vault floors share burns, so an exact-debt repayment can strand one dust share — leaving the position open while the buy-back still charges the trader; the extra wei makes the burn cap at all shares, and the router hard-requires closure), repay (collateral pays out to the trader), buy back exactly what the repayment consumed under a `maxCollateralIn` bound, and settle the refunded remainder — the router ends every close holding nothing. Notably the oracle's worse-of pricing self-defends here: the router's own flash swap moves the spot price, and origination values collateral at the pre-swap filtered price, so the construction cannot lever against its own impact.
...
Implemented, green, and deployed: 97 tests (fuzzed library units, vault accounting, 28 lifecycle scenarios, 4 native-ETH pool scenarios, 9 periphery scenarios, randomized invariants), gas snapshot checked in, deploy script (address mining + CREATE2 + linked libraries) verified end-to-end, live on Unichain Sepolia — addresses in the [README](README.md).
